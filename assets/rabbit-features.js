(function(){
  ...
  function renderHistory(){
    const list=readHistory(),side=document.querySelector('.rqf-side');if(!side)return;
    let card=byId('rabbitAlertHistory');if(!card){card=document.createElement('details');card.id='rabbitAlertHistory';card.className='rqf-side-card rabbit-alert-history';side.appendChild(card)}
    card.innerHTML=`<summary>今日提醒 <span class="rabbit-alert-count">${list.length}</span></summary><div class="rabbit-alert-list">${list.length?list.slice(0,12).map(item=>`<div class="rabbit-alert-item" data-code="${esc(item.code)}"><em>${esc(item.time)}</em><b>${esc(item.name)} · ${esc(item.signal)}</b><span>${item.price?item.price.toFixed(2):'--'}</span></div>`).join(''):'<div class="rabbit-alert-empty">今天还没有提醒</div>'}</div>`;
